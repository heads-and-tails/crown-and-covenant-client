...
class TacticalController:
    def path(self, observation: dict, start: dict, target: dict) -> list[dict]:
        return [
            {"x": x, "y": y} for x, y in routes(observation, pos(start))(pos(target))
        ]
    ...
    def plan(self, observation: dict, intent: dict | None = None) -> dict:
        ...
        units = o["rules"]["units"]
        strength = lambda a: sum(
            a["troops"].get(t, 0) * units[t]["power"] for t in TROOPS
        )
        allies = {you}
        own = sorted(
            (a for a in o["armies"] if a["owner"] == you),
            key=lambda a: (-strength(a), a["id"]),
        )
        ...
        assigned, moves = set(), []
        stance = intent.get(
            "stance", "defend" if me.get("style") == "defensive" else "expand"
        )
        for army in own:
            route = routes(o, pos(army), blocked)
            candidates = []
            for site in o["structures"]:
                if site["owner"] in allies or site["owner"] in intent.get(
                    "avoidPlayers", []
                ):
                    continue
                path = route(pos(site))
                defense = sum(
                    strength(a) for a in hostile if pos(a) == pos(site)
                ) + strength({"troops": site["garrison"]})
                defense *= 1.6 if site["kind"] == "castle" else 1
                value = (24 if site["kind"] == "castle" else 7) + (
                    6 if site["owner"] is None else 0
                )
                ...
                if (
                    path
                    and site["id"] not in assigned
                    and strength(army)
                    > defense * (1.0 if me.get("style") == "aggressive" else 1.2)
                ):
                    candidates.append((value / (len(path) + 1), site))
            destination = intent.get("targets", {}).get(army["id"]) or next(
                (
                    v
                    for v in intent.get("armyObjectives", [])
                    if v.get("armyId") == army["id"]
                ),
                None,
            )
            ...
            if not destination and (
                stance == "defend" or intent.get("defensivePriorities")
            ):
                destination = next(
                    (
                        s
                        ...
                        and (
                            stance == "defend"
                            or s["id"] in intent.get("defensivePriorities", [])
                        )
                        and any(
                            max(abs(a["x"] - s["x"]), abs(a["y"] - s["y"])) <= 3
                            for a in hostile
                        )
                    ),
                    None,
        ...
        # If an assigned route has become unsafe, explicitly stop it instead of retaining it.
        ordered = {m["armyId"] for m in moves}
        for a in own:
            if a.get("route") and a["id"] not in ordered:
                moves.append({"armyId": a["id"], "x": a["x"], "y": a["y"], "route": []})
        castles = sorted(
            (s for s in o["structures"] if s["kind"] == "castle" and s["owner"] == you),
            key=lambda s: s["id"],
        )
        ...
        elif sum(a["troops"]["knight"] for a in hostile) > sum(
            a["troops"]["archer"] for a in hostile
        ):
        ...
        if composition:
            ...
            desired = sorted(
                (t for t in TROOPS if composition.get(t, 0) > 0),
                key=lambda t: -(composition[t] / 100 - counts[t] / total),
            )
        ...
        for castle in castles:
            ...
            for troop in preference:
                ...
                count = min(
                    o["rules"]["productionCapacity"],
                    *(budget[r] // qty for r, qty in cost.items()),
                )
            ...
            production.append(
                {
                    "castleId": castle["id"],
                    "troop": selected if count else None,
                    "count": max(1, count),
                }
            )
        return {
