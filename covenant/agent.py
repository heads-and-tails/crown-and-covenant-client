...
class Agent:
    """Override any callbacks you need. Context methods submit immediate actions."""

    def on_turn(self, ctx):
        return self.decide(ctx.get_state())

    def on_message(self, ctx, message):
        pass

    def on_events(self, ctx, events):
        return self.diplomacy(ctx.get_state())
    ...
    def diplomacy(self, o):
        ...
        for offer in o["offers"]:
            if (
                ...
                or offer["expiresTurn"] <= o["turn"]
            ):
                continue
            accept = False
            if offer["kind"] == "trade":
                accept = sum(offer["give"].values()) >= sum(
                    offer["want"].values()
                ) * 0.75 and all(
                    o["treasury"][r] >= offer["want"].get(r, 0) for r in RESOURCES
                )
            if accept:
                actions.append(
                    {
            ...
            actions += [
                {
                    ...
                    "data": {
                        "to": p["id"],
                        "text": f"Greetings from {me['name']}. I am securing nearby resources and welcome fair trades.",
                    },
                }
                for p in opponents
            ]
        if o["turn"] % 3 == 0:
            surplus = max(
                (r for r in RESOURCES if r != "grain"), key=lambda r: o["treasury"][r]
            )
            need = min(
                (r for r in RESOURCES if r not in ("grain", surplus)),
                key=lambda r: o["treasury"][r],
            )
            seller = next(
                (
                    p
                    for p in opponents
                    if any(
                        s["owner"] == p["id"] and s.get("resource") == need
            ...
            if (
                seller
                and o["treasury"][surplus] >= 8
                and o["treasury"][need] < 8
                and not any(
                    f["from"] == you
                    and f["to"] == seller["id"]
